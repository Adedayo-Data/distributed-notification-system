...
class RabbitMQPublisher:
    ...
    async def _setup_exchanges(self):
        """Set up exchanges only - consumers declare their own queues"""
        try:
            # Declare main notifications exchange
            self.exchange = await self.channel.declare_exchange(
                settings.RABBITMQ_EXCHANGE,  # "notifications.direct"
                type=aio_pika.ExchangeType.DIRECT,
                durable=True
            )
            ...
            logger.error(f"Failed to setup exchanges: {str(e)}")
            raise
